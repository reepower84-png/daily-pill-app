...
그래서 이 앱은 형제 앱의 토큰을 **함부로 물려받지 않습니다.** 버튼 앱들이 같은 주소에
...
그래서 물려받기 전에 **쓰기까지 되는지 떠봅니다.** 있는 파일에  없이 PUT 을 던지면
아무것도 써지지 않으면서 거절되는 이유가 갈립니다 —  은 쓰기 권한이 없는 것이고,
 는 권한은 있고 sha 가 없어서 거절된 것입니다. ( 의 
로는 못 가립니다 — 재 보니 읽기 전용 토큰에도, 공개 저장소에도  로 옵니다.
그 값은 토큰의 권한이 아니라 계정이 그 저장소에서 가진 역할을 비춥니다.)
